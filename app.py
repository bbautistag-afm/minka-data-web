...
st.info("Sube tus archivos PDF. El sistema extraerá DNI, nombres, notas y situación final automáticamente.")
...
def procesar_acta_universal(pdf_file):
    ...
    # --- LÓGICA DE NOMENCLATURA (Tu código original) ---
...
if archivos_cargados:
    if st.button("🚀 INICIAR PROCESAMIENTO MASIVO"):
        ...
        if lista_maestra:
            ...
            st.balloons()
            st.success(f"🏆 ¡Procesamiento completado! {len(lista_maestra)} alumnos encontrados.")
            st.download_button(
                label="📥 Descargar Excel de UGEL MELGAR",
                data=output.getvalue(),
                file_name="MINKA_DATA_MELGAR_FINAL.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
        else:
            st.error("❌ No se pudieron extraer datos. Revisa el formato de los nombres de tus archivos.")
